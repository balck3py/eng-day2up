...
vi.mock('./dictapi', () => ({
  readCachedPhonetics: vi.fn(async () => ({
    phoneticUs: '/us/', phoneticUk: '/uk/',
    audioUs: 'a-us.mp3', audioUk: 'a-uk.mp3',
  })),
}))

const mockedReadCachedPhonetics = vi.mocked(readCachedPhonetics)
...
describe('lookupWord', () => {
  it('第一级：精确命中', async () => {
    ...
    const { detail: r } = await lookupWord(db, 'apple')
  ...
  it('第二级：经 dict_lemma 命中原型', async () => {
    ...
    const { detail: r } = await lookupWord(db, 'said')
  ...
  it('第三级：后缀规则兜底', async () => {
    ...
    const { detail: r } = await lookupWord(db, 'running')
  ...
  it('第三级：单一候选命中不受排序调整影响（studies → study）', async () => {
    ...
    const { detail: r } = await lookupWord(db, 'studies')
    expect(r.matchedFrom).toBe('suffix')
  ...
  it('第三级：单一候选命中不受排序调整影响（worked → work）', async () => {
    ...
    const { detail: r } = await lookupWord(db, 'worked')
    expect(r.matchedFrom).toBe('suffix')
  ...
  it('第三级：多个候选都是真词时优先取哑音 e 还原（caring → care，而不是 car），且不依赖数据库返回行序', async () => {
    ...
    const { detail: r } = await lookupWord(db, 'caring')
    expect(r.matchedFrom).toBe('suffix')
  ...
  it('第三级：过去式候选同理（cared → care，而不是 car），且不依赖数据库返回行序', async () => {
    const db = fakeDbScrambled({ car: CAR, care: CARE }, {}, ['car', 'care'])
    const { detail: r } = await lookupWord(db, 'cared')
    expect(r.matchedFrom).toBe('suffix')
    expect(r.word).toBe('care')
  })

  it('第三级：命中后用的是解析出的原型词去查缓存音标，而不是原始输入', async () => {
    const db = fakeDbScrambled({ car: CAR, care: CARE }, {}, ['car', 'care'])
    await lookupWord(db, 'caring')
    expect(mockedReadCachedPhonetics).toHaveBeenCalledWith(db, 'care')
    expect(mockedReadCachedPhonetics).not.toHaveBeenCalledWith(db, 'caring')
  })
  ...
  it('第二级：一个 form 对应多个 lemma 时按字典序取最小者作为确定性选择，且不依赖数据库返回行序', async () => {
    ...
    const { detail: r } = await lookupWord(db, 'foo')
    expect(r.matchedFrom).toBe('lemma')
    expect(r.word).toBe('apple')
  })

  it('exact 与 lemma 并行发起，但 exact 命中时仍优先于 lemma（即便 lemma 也命中）', async () => {
    // apple 既是精确命中，也（人为构造地）作为某个 form 的 lemma；
    // 验证并行发起查询不会改变谁赢——exact 必须赢。
    const db = fakeDb({ apple: APPLE, run: RUN }, { apple: ['run'] })
    const { detail: r } = await lookupWord(db, 'apple')
    expect(r.matchedFrom).toBe('exact')
    expect(r.word).toBe('apple')
  })
  ...
  it('第四级：全部未命中', async () => {
    const db = fakeDb({}, {})
    const { detail: r } = await lookupWord(db, 'zzzznotaword')
    expect(r.matchedFrom).toBe('none')
    expect(r.senses).toEqual([])
    expect(r.phonetic).toBeNull()
  })

  it('未命中时若音标已缓存仍会返回', async () => {
    const db = fakeDb({}, {})
    const { detail: r } = await lookupWord(db, 'zzzznotaword')
  ...
  it('音标缓存未命中时返回空音标，并报告需要后台补抓的词', async () => {
    mockedReadCachedPhonetics.mockResolvedValueOnce(null)
    const db = fakeDb({ apple: APPLE }, {})
    const { detail, refreshPhoneticsKey } = await lookupWord(db, 'apple')
    expect(detail.phoneticUs).toBeNull()
    expect(detail.phoneticUk).toBeNull()
    expect(refreshPhoneticsKey).toBe('apple')
  })

  it('音标缓存命中时不需要后台补抓', async () => {
    const db = fakeDb({ apple: APPLE }, {})
    const { refreshPhoneticsKey } = await lookupWord(db, 'apple')
    expect(refreshPhoneticsKey).toBeNull()
  })
  ...
  it('tag 为空串时 tags 为空数组', async () => {
    const db = fakeDb({ run: RUN }, {})
    const { detail: r } = await lookupWord(db, 'run')
    expect(r.tags).toEqual([])
  })

  it('oxford 转为布尔', async () => {
    const db = fakeDb({ apple: APPLE }, {})
    expect((await lookupWord(db, 'apple')).detail.oxford).toBe(true)
  })

  it('保留 ECDICT 单一音标作为保底', async () => {
    const db = fakeDb({ apple: APPLE }, {})
    expect((await lookupWord(db, 'apple')).detail.phonetic).toBe('/ˈæpl/')
  })

  it('空输入直接返回未命中，不查库，也不需要后台补抓', async () => {
    const db = fakeDb({}, {})
    const { detail: r, refreshPhoneticsKey } = await lookupWord(db, '!!!')
    expect(r.matchedFrom).toBe('none')
    expect(refreshPhoneticsKey).toBeNull()
  })
})
